...
def process_loan_repayment(loan):
    ...
    try:
        ...
        updated_reps = []  # 👈 store updates instead of saving one-by-one
        ...
        for rep in repayments:
            ...
            updated_reps.append(rep)  # 👈 collect

            last_date = rep_date

        # ✅ SINGLE BULK UPDATE (fixes I/O error)
        if updated_reps:
            LoanRepayment.objects.bulk_update(
                updated_reps,
                ['paid_to_interest', 'paid_to_principal']
            )
        ...
        logger.error(f"🔥 Loan processing failed for Loan {loan.id}: {e}")
        # Optional: raise if you want strict failure
        # raise